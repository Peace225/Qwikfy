...
import CheckoutPopup from "../components/CheckoutPopup"; // 🔥 Importe ton popup
...
export default function Destockage() {
  ...
  return (
    <section className="bg-[#62d4f9] py-6 px-4 mt-6">
      <h2 className="text-lg font-bold text-white mb-4">Déstockage</h2>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {produitsDestockage.map((item, i) => (
          <div
            key={i}
            onClick={() => setSelectedProduct(item)} // 🔥 ouvrir le popup au clic
